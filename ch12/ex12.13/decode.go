...
Exercise 12.13: Modify the S-expression encoder (§12.4) and decoder (§12.6) so
that they honor the sexpr:"..." field tag in a similar manner to
encoding/json (§4.5).
...
//!-read
type structFieldNameMapperT map[string]map[string]string
var structFieldNameMapper structFieldNameMapperT

func getStructFieldNameByTag(v reflect.Value, field string) string {

	if structFieldNameMapper == nil {
		structFieldNameMapper = make(structFieldNameMapperT)
	}

	sName := v.Type().String()
	s, ok := structFieldNameMapper[sName]
	if !ok {
		...
		for i := 0; i < v.NumField(); i++ {
			...
			m[name] = fieldInfo.Name
		}
		structFieldNameMapper[sName] = m
		s = m
	}
...
func readList(lex *lexer, v reflect.Value) {
	...
	case reflect.Struct: // ((name value) ...)
		for !endList(lex) {
			...
			name := lex.text()
			lex.next()
			name = getStructFieldNameByTag(v, name)
