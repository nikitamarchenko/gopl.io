...
Exercise 12.10: Extend sexpr.Unmarshal to handle the booleans, floating-point
numbers, and interfaces encoded by your solution to Exercise 12.3. (Hint: to
decode interfaces, you will need a mapping from the name of each supported type
to its reflect.Type.)
...
type interfacesT map[string]reflect.Type

var interfaces interfacesT

func init() {
	interfaces = make(interfacesT)
}

func RegisterInterface(name string, t reflect.Type) {
	interfaces[name] = t
}
...
func read(lex *lexer, v reflect.Value) {
	...
	panic(fmt.Sprintf("unexpected token %q, %q", lex.text(), lex.token))
}
...
func readList(lex *lexer, v reflect.Value) {
	...
	case reflect.Interface: // (name value)
		...
		if err != nil {
			panic(fmt.Sprintf("can't decode interface name must be quoted %v", lex.text()))
		}
		lex.next()
		t, ok := interfaces[name]
		if !ok {
